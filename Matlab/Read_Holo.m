...
fd = fopen(filename, 'r');
offset = 65;
frame_batch = zeros(frame_width, frame_height, num_frames, 'uint8'); % single 
...
for i = 1:num_frames
    ...
    if bit_depth == 8
        ...
        frame_batch(width_range, height_range, i) = reshape(fread(fd, frame_width * frame_height, 'uint16=>single', endian), frame_width, frame_height);
    end 
   
end

implay(frame_batch(:,:,:),30); %30 fps 
fclose(fd);
